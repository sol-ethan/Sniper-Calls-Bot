...
import SniperCallsbot_commons.asyncio_tools as asyncio_tools
...
class TaskManager:
    """TaskManager class:
        - Create, manage and stop SniperCallsbot tasks
    """
    ...
    async def start_tools_tasks(self):
        ...
        if self.SniperCallsbot.community_handler:
            task_list.append(self.SniperCallsbot.community_handler.start_community_task())

        self.SniperCallsbot.async_loop = self.async_loop
        self.ready = True
    ...
    def run_bot_in_thread(self, coroutine):
        ...
        self.logger.debug("Stopped SniperCallsBot main loop")
    ...
    def run_forever(self, coroutine):
        if constants.RUN_IN_MAIN_THREAD:
            ...
            self.loop_forever_thread = threading.Thread(
                ...
                name="SniperCallsBot Main Thread")
            self.loop_forever_thread.start()
            if sys.version_info.minor >= 9:
                # only required for python 3.9 +
                self.loop_forever_thread.join()

    def stop_tasks(self, stop_SniperCallsbot=True):
        self.logger.info("Stopping tasks...")

        async def stop_timeout(timeout):
            await asyncio.wait_for(self.SniperCallsbot.stopped.wait(), timeout)

        stop_coroutines = []
        if stop_SniperCallsbot:
            allowed_seconds_to_stop = 10
            stop_coroutines.append(self.SniperCallsbot.stop())
        ...
        if self.SniperCallsbot.community_handler:
            stop_coroutines.append(self.SniperCallsbot.community_handler.stop_task())

...
import src.updater.updater as updater_class
...
class BinaryUpdater(updater_class.Updater):
    ...
    async def update_impl(self) -> bool:
        ...
        self.logger.error(f"Please manually update your SniperCallsBot executable from this page: "
    ...
    async def _download_binary(self):
        ...
        if matching_asset_binary is None:
            ...
            return None
        self.logger.info(f"Start downloading SniperCallsBot update at {new_binary_file_url}")
        ...
        self.logger.info(f"SniperCallsBot update downloaded successfully")
    ...
    def _give_execution_rights(self, new_binary_file):
        if os_util.get_os() is not commons_enums.PlatformsName.WINDOWS:
            self.logger.info(f"Adding execution rights to updated SniperCallsBot binary")

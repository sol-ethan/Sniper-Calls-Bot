...
import SniperCallsbot_tentacles_manager.cli as tentacles_manager_cli
import SniperCallsbot_tentacles_manager.constants as tentacles_manager_constants

# make tentacles importable
sys.path.append(os.path.dirname(sys.executable))

import src.SniperCallsbot as SniperCallsbot_class
import src.commands as commands
import src.configuration_manager as configuration_manager
import src.SniperCallsbot_backtesting_factory as SniperCallsbot_backtesting
import src.constants as constants
import src.disclaimer as disclaimer
import src.logger as SniperCallsbot_logger
import src.community as SniperCallsbot_community
import src.limits as limits


def update_config_with_args(starting_args, config: configuration.Configuration, logger):
    try:
        import SniperCallsbot_backtesting.constants as backtesting_constants
    except ImportError as e:
        logging.get_logger().error(
            "Can't start backtesting without the SniperCallsbot_backtesting package properly installed.")
        raise e
...
def _log_environment(logger):
    try:
        ...
        logger.info(f"Running {bot_type} SniperCallsBot on {os_util.get_current_platform()} "
                    f"with {os_util.get_SniperCallsbot_type()} "
...
async def _apply_community_startup_info_to_config(logger, config, community_auth):
    ...
    except SniperCallsbot_community.errors.BotError:
        return
...
async def _get_authenticated_community_if_possible(config, logger):
    ...
    SniperCallsbot_community.IdentifiersProvider.use_environment_from_config(config)
    community_auth = SniperCallsbot_community.CommunityAuthentication.create(config)
    try:
...
def _validate_config(config, logger):
    try:
        ...
        if configuration_manager.migrate_from_previous_config(config):
            ...
            logger.error("SniperCallsBot can't repair your config.json file: invalid format: " + str(err))
            raise errors.ConfigError from err


def _repair_with_default_profile(config, logger):
    logger.error("SniperCallsBot can't start without a valid profile configuration. Selecting default profile ...")
...
def _load_or_create_tentacles(community_auth, config, logger):
    ...
    if os.path.isfile(tentacles_manager_constants.USER_REFERENCE_TENTACLE_CONFIG_FILE_PATH):
        ...
        logger.info("SniperCallsBot tentacles can't be found. Installing default tentacles ...")
        commands.run_tentacles_install_or_update(community_auth, config)
        config.load_profiles_if_possible_and_necessary()


def start_SniperCallsbot(args):
    logger = None
    try:
        if args.version:
        ...
        logger = SniperCallsbot_logger.init_logger()
        startup_messages = []
        ...
        SniperCallsbot_community.register_error_uploader(constants.ERRORS_POST_ENDPOINT, config)

        # Apply config limits if any
        startup_messages += limits.apply_config_limits(config)

        # create SniperCallsBot instance
        if args.backtesting:
            bot = SniperCallsbot_backtesting.SniperCallsBotBacktestingFactory(config,
            ...
            bot = SniperCallsbot_class.SniperCallsBot(config, community_authenticator=community_auth,
        ...
        logger.error("SniperCallsBot can't start without a valid " + common_constants.CONFIG_FILE
                     + " configuration file.\nError: " + str(e) + "\nYou can use " +
                     constants.DEFAULT_CONFIG_FILE + " as an example to fix it.")
        os._exit(-1)

    except errors.NoProfileError:
        logger.error("Missing default profiles. SniperCallsBot can't start without a valid default profile configuration. "
                     ...
                     f"folder or start SniperCallsBot with the following arguments: tentacles --install --all")
        os._exit(-1)

    except ModuleNotFoundError as e:
        if 'tentacles' in str(e):
            logger.error("Impossible to start SniperCallsBot, tentacles are missing.\nTo install tentacles, "
        ...
        logger.error("SniperCallsBot can't start without a valid  configuration file.\n"
...
def SniperCallsbot_parser(parser):
    parser.add_argument('-v', '--version', help='Show SniperCallsBot current version.',
                        action='store_true')
    parser.add_argument('-s', '--simulate', help='Force SniperCallsBot to start with the trader simulator only.',
                        action='store_true')
    parser.add_argument('-u', '--update', help='Update SniperCallsBot to latest version.',
                        action='store_true')
    parser.add_argument('-rts', '--reset-trading-history', help='Force the traders to reset their history. They will '
                                                                ...
                                                                'fresh new portfolio.',
                        action='store_true')
    parser.add_argument('-b', '--backtesting', help='Start SniperCallsBot in backesting mode using the backtesting '
    ...
    parser.add_argument('-nw', '--no_web', help="Don't start SniperCallsBot web interface.",
                        action='store_true')
    parser.add_argument('-nl', '--no_logs', help="Disable SniperCallsBot logs in backtesting.",
                        action='store_true')
    parser.add_argument('-nt', '--no-telegram', help='Start SniperCallsBot without telegram interface, even if telegram '
                                                     'credentials are in config. With this parameter, your SniperCallsbot '
    ...
    parser.add_argument('--encrypter', help="Start the exchange api keys encrypter. This tool is useful to manually add"
                        ...
                        action='store_true')
    parser.add_argument('--identifier', help="SniperCallsBot community identifier.", type=str, nargs=1)
    parser.add_argument('-o', '--strategy_optimizer', help='Start SniperCallsbot strategy optimizer. This mode will make '
                                                           'SniperCallsbot play backtesting scenarii located in '
                        ...
                        nargs='+')
    parser.set_defaults(func=start_SniperCallsbot)

    # add sub commands
    subparsers = parser.add_subparsers(title="Other commands")

    # tentacles manager
    tentacles_parser = subparsers.add_parser("tentacles", help='Calls SniperCallsBot tentacles manager.\n'
...
def start_background_SniperCallsbot_with_args(version=False,
                                       update=False,
                                       encrypter=False,
    ...
    if in_subprocess:
        bot_process = multiprocessing.Process(target=start_SniperCallsbot, args=(args,))
        bot_process.start()
        return bot_process
    else:
        return start_SniperCallsbot(args)


def main(args=None):
    if not args:
        args = sys.argv[1:]
    parser = argparse.ArgumentParser(description='SniperCallsBot')
    SniperCallsbot_parser(parser)

    MIN_TENTACLE_MANAGER_VERSION = "1.0.10"

    # check compatible tentacle manager
    try:
        from SniperCallsbot_tentacles_manager import VERSION

        if packaging_version.Version(VERSION) < packaging_version.Version(MIN_TENTACLE_MANAGER_VERSION):
            print("SniperCallsBot requires SniperCallsBot-Tentacles-Manager in a minimum version of " + MIN_TENTACLE_MANAGER_VERSION +
                  " you can install and update SniperCallsBot-Tentacles-Manager using the following command: "
                  "python3 -m pip install -U SniperCallsBot-Tentacles-Manager", file=sys.stderr)
            sys.exit(-1)
    except ImportError:
        print("SniperCallsBot requires SniperCallsBot-Tentacles-Manager, you can install it using "
              "python3 -m pip install -U SniperCallsBot-Tentacles-Manager", file=sys.stderr)
        sys.exit(-1)

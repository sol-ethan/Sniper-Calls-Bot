...
import SniperCallsbot_evaluators.enums as evaluator_enums

import SniperCallsbot_services.constants as service_constants

import SniperCallsbot_trading.api as trading_api

import src.community.models.community_fields as community_fields
...
class CommunityManager:
    ...
    def __init__(self, SniperCallsbot_api):
        self.SniperCallsbot_api = SniperCallsbot_api
        self.edited_config: configuration.Configuration = SniperCallsbot_api.get_edited_config(dict_only=False)
        ...
        self.session = SniperCallsbot_api.get_aiohttp_session()
        try:
    ...
    def _init_community_config(self):
        ...
        self.exchange_managers = trading_api.get_exchange_managers_from_exchange_ids(
            self.SniperCallsbot_api.get_exchange_manager_ids())
    ...
    @staticmethod
    def background_get_id_and_register_bot(SniperCallsbot_api):
        community_manager = CommunityManager(SniperCallsbot_api)
    ...
    async def _update_session(self, retry_on_error=True):
        self.current_config[community_fields.CommunityFields.CURRENT_SESSION.value][
            community_fields.CommunityFields.UP_TIME.value] = int(time.time() - self.SniperCallsbot_api.get_start_time())
        self.current_config[community_fields.CommunityFields.CURRENT_SESSION.value][
    ...
    def _get_bot_community(self):
        return {
            ...
            community_fields.CommunityFields.CURRENT_SESSION.value: {
                community_fields.CommunityFields.STARTED_AT.value: int(self.SniperCallsbot_api.get_start_time()),
                community_fields.CommunityFields.UP_TIME.value: int(time.time() - self.SniperCallsbot_api.get_start_time()),
                ...
                community_fields.CommunityFields.TYPE.value: os_util.get_SniperCallsbot_type(),
    ...
    def _get_traded_volumes(self):
        ...
        if self.has_real_trader:
            trades = []
            for exchange_manager in self.exchange_managers:
                trades += trading_api.get_trade_history(exchange_manager, since=self.SniperCallsbot_api.get_start_time())
            for trade in trades:
    ...
    def _get_eval_config(self):
        tentacle_setup_config = self.SniperCallsbot_api.get_tentacles_setup_config()
        # trading mode
        config_eval = []
        if (trading_mode := self.SniperCallsbot_api.get_trading_mode()) is not None:

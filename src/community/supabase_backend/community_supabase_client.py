...
import SniperCallsbot_commons.profiles as commons_profiles
...
import src.community.supabase_backend.supabase_client as supabase_client
import src.community.supabase_backend.configuration_storage as configuration_storage
...
class CommunitySupabaseClient(supabase_client.AuthenticatedAsyncSupabaseClient):
    """
    SniperCallsbot Community layer added to supabase_client.AuthenticatedSupabaseClient
    """
    ...
    async def get_user(self) -> dict:
        try:
            ...
            if "missing" in str(err):
                raise errors.EmailValidationRequiredError(err) from err
            raise authentication.AuthenticationError("Please re-login to your SniperCallsBot account") from err

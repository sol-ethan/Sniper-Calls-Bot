...
import SniperCallsbot_commons.logging as common_logging
import SniperCallsbot_commons.channels_name as channels_name
import SniperCallsbot_commons.pretty_printer as pretty_printer

import SniperCallsbot_evaluators.evaluators.channel as evaluator_channels

import SniperCallsbot_trading.exchange_channel as exchanges_channel
...
def init_logger():
    try:
        ...
        print(
            "Impossible to start SniperCallsBot: the logging configuration can't be found in '"
            + constants.LOGGING_CONFIG_FILE
            + "' please make sure you are running SniperCallsBot from its root directory."
        )
        os._exit(-1)

    logger = logging.getLogger("SniperCallsBot Launcher")
    ...
    try:
        ...
        print(
            "Impossible to start SniperCallsBot: the logging file is locked, this is probably due to another running "
            "SniperCallsBot instance."
        )
        os._exit(-1)
...
def init_bot_channel_logger():
    ...
    BOT_CHANNEL_LOGGER = common_logging.get_logger("SniperCallsBot Channel")
...
async def init_exchange_chan_logger(exchange_id):
    await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.OHLCV_CHANNEL.value,
                                     exchange_id).new_consumer(
        ohlcv_callback, priority_level=LOGGER_PRIORITY_LEVEL
    )
    await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.BALANCE_CHANNEL.value,
                                     exchange_id).new_consumer(
        balance_callback, priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value
    )
    await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.TRADES_CHANNEL.value,
                                     exchange_id).new_consumer(
        trades_callback, priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value
    )
    await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.LIQUIDATIONS_CHANNEL.value,
                                     exchange_id).new_consumer(
        liquidations_callback, priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value
    )
    await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.POSITIONS_CHANNEL.value,
                                     exchange_id).new_consumer(
        positions_callback, priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value
    )
    await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.ORDERS_CHANNEL.value,
                                     exchange_id).new_consumer(
    ...
    if constants.ENV_TRADING_ENABLE_DEBUG_LOGS:
        await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.RECENT_TRADES_CHANNEL.value,
                                         exchange_id).new_consumer(
            recent_trades_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.FUNDING_CHANNEL.value,
                                         exchange_id).new_consumer(
            funding_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.TICKER_CHANNEL.value,
                                         exchange_id).new_consumer(
            ticker_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.MINI_TICKER_CHANNEL.value,
                                         exchange_id).new_consumer(
            mini_ticker_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.ORDER_BOOK_CHANNEL.value,
                                         exchange_id).new_consumer(
            order_book_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.ORDER_BOOK_TICKER_CHANNEL.value,
                                         exchange_id).new_consumer(
            order_book_ticker_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.KLINE_CHANNEL.value,
                                         exchange_id).new_consumer(
            kline_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.MARK_PRICE_CHANNEL.value,
                                         exchange_id).new_consumer(
            mark_price_callback, priority_level=LOGGER_PRIORITY_LEVEL
        )
        await exchanges_channel.get_chan(channels_name.SniperCallsBotTradingChannelsName.BALANCE_PROFITABILITY_CHANNEL.value,
                                         exchange_id).new_consumer(
...
async def init_evaluator_chan_logger(matrix_id: str):
    await evaluator_channels.get_chan(channels_name.SniperCallsBotEvaluatorsChannelsName.MATRIX_CHANNEL.value,
                                      matrix_id).new_consumer(
        matrix_callback, priority_level=LOGGER_PRIORITY_LEVEL
    )
    await evaluator_channels.get_chan(channels_name.SniperCallsBotEvaluatorsChannelsName.EVALUATORS_CHANNEL.value,
                                      matrix_id).new_consumer(
        evaluators_callback, priority_level=LOGGER_PRIORITY_LEVEL
    )


async def init_SniperCallsbot_chan_logger(bot_id: str):
    await channel_instances.get_chan_at_id(constants.SniperCallsBOT_CHANNEL, bot_id).new_consumer(
        SniperCallsbot_channel_callback,
        priority_level=LOGGER_PRIORITY_LEVEL,
        bot_id=bot_id,
        subject=[enums.SniperCallsBotChannelSubjects.NOTIFICATION.value, enums.SniperCallsBotChannelSubjects.ERROR.value]
    )
...
async def SniperCallsbot_channel_callback(
        bot_id: str,
        subject: str,
        action: str,
        data: dict
    ...
    BOT_CHANNEL_LOGGER.debug(
        f"SniperCallsBOT_CHANNEL : SUBJECT = {subject} || ACTION = {action} || DATA = {data} "
    )
